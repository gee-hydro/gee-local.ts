...
if (length(args) != 2L) {
  stop("用法：Rscript examples/SurfaceWater/plot-china-dswx-availability.R input_dir output.png")
}

files <- sort(list.files(
  args[[1]],
  pattern = "China_DSWX_valid_days_[0-9]{4}_1-24deg\\.tif$",
  full.names = TRUE
))
if (!length(files)) stop("未找到有效观测日数 TIF")

years <- sub(".*days_([0-9]{4})_.*", "\\1", files)
...
annual_plot <- metric_plot(annual, "年有效观测日数")
monthly_plot <- metric_plot(monthly, "平均每月有效观测日数")
if (length(files) == 1L) {
...
plot <- plot + plot_annotation(
  title = "中国 OPERA DSWx-HLS 有效观测日数",
  subtitle = "1/24° 网格；同日所有卫星与 granule 合并"
)
